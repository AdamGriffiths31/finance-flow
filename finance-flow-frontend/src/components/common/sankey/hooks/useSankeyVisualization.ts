...
export const useSankeyVisualization = ({
  ...
  useEffect(() => {
    ...
    let g = svg.select<SVGGElement>('g.sankey-container');
    
    if (g.empty() || needsFullRedraw) {
      if (needsFullRedraw) {
        svg.selectAll('*').remove();
      }
      g = svg.append('g')
